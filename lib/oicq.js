...
async function parseSSO(buf) {
    ...
    if (retcode !== 0) {
        const retmsg_len = buf.readUInt32BE(12);
        const retmsg = String(buf.slice(16, 16 + retmsg_len - 4));
    ...
    offset += 4;
    len = buf.readUInt32BE(offset); // length of reserved
    const reserved = buf.slice(offset + 4, offset + len);
    let payload;
    if (flag === 0) {
