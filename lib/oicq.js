...
const Reader = require("./algo/reader");
...
const { int32ip2str, getUploadChannel, checkIPConnectivity } = require("./service");
 ...
 * @this {Client}
 */
async function getStrangerList() {
    ...
    try {
        const blob = await this.sendOidb("OidbSvc.0x5d2_0", body);
        const rsp = pb.decode(blob)[4][2];
        if (!Array.isArray(rsp[2]))
            rsp[2] = [rsp[2]];
        for (const v of rsp[2]) {
            if (this.sl.has(v[1])) {
                this.sl.get(v[1]).nickname = String(v[2]);
                this.sl.get(v[1]).group_id = undefined;
            } else {
                this.sl.set(v[1], {
                    user_id: v[1],
                    nickname: String(v[2]),
    ...
    if (nested[1] === 2 && nested[2]) {
        ...
        for (let v of decoded[3]) {
            if (v[1] === 10) {
                ...
                checkIPConnectivity(this.storage.ip, this.storage.port).then((res) => {
                    if (res) return;
...
function pushNotifyListener(blob) {
    ...
    try {
        var nested = jce.decode(blob.slice(4));
    } catch {
        var nested = jce.decode(blob.slice(15));
    }
    switch (nested[5]) {
    case 33: //群员入群
    case 38: //建群
 ...
 * NT强制下线通知
 */
function ntOfflineListener(blob) {
    const decoded = pb.decode(blob);
    this.emit("internal.kickoff", {
        type: "PushForceOffline",
        info: `[${decoded[4]}]${decoded[3]}(${decoded[5]})`,
    });
}
...
function kickoffListener(data) {
    ...
    if (data.info.includes("如非本人操作") ||
        data.info.includes("如果这不是你本人操作") ||
        data.info.includes("密码可能已经泄露") ||
        data.info.includes("当前设备已下线")
    ) {
        sub_type = "kickoff";
    ...
    if (sub_type !== "kickoff") this._wt.deleteToken();
    this.em("system.offline." + sub_type, { message: data.info });
}

Client.prototype._register = async function () {
    if (!await this._wt.register()) {
        if (++this._registerRetryCount > 10) {
            this.logger.error("register重试超过10次，停止重试");
            this._registerRetryCount = 0;
            this.terminate();
            return this.em("system.login.error", { code: -1, message: "上线注册失败，请检查token与协议类型是否匹配" });
        }
...
Client.prototype._startHeartbeat = function () {
    ...
    this._heartbeat = setInterval(async () => {
        ...
        this._wt.heartbeat();
        if (this.config.useNT) {
            if (this._nt_hb_var++ >= 5) {
                this._nt_hb_var = 0;
                if (!await this._wt.ssoHeartbeat() && this.isOnline()) {
                    this.logger.warn("SsoHeartBeat timeout!");
                    if (!await this._wt.ssoHeartbeat() && this.isOnline())
                        this._socket.destroy();
                }
            }
        }
        if (timestamp() - this.sig.qsign_token_time >= (60 * 60)) {
            this.sig.qsign_token_time = timestamp();
            this.refreshQSignToken();
        }
        if (!this.storage.port) {
            try {
                await getUploadChannel.call(this);
            } catch {
                this.logger.warn("getUploadChannel failed");
            }
        }
    }, 30000);
};
...
async function parseSSO(buf) {
    ...
    const header = new Reader(stream.readWithLength());
    const headlen = buf.readInt32BE();
    const seq = header.read(4).readInt32BE();
    const retcode = header.read(4).readInt32BE();
    if (retcode !== 0) {
        const retmsg = String(header.readWithLength());
        const error = new Error("unsuccessful retcode: " + retcode + " retmsg: " + retmsg + " seq: " + seq);
        if (this.handlers.has(seq))
            this.handlers.get(seq)(null, error);
        throw error;
    }
    const cmd = String(header.readWithLength());

    if (cmd === "Heartbeat.Alive") {
        return {
            seq, cmd, payload: BUF0, reserved: BUF0
        };
    }

    const session_id = header.readWithLength();
    const flag = header.read(4).readInt32BE();
    const reserved = header.readWithLength();
    let payload;
    if (flag === 0) {
        ...
        const error = new Error("unknown compressed flag: " + flag + " cmd: " + cmd + " seq: " + seq);
        if (this.handlers.has(seq))
            this.handlers.get(seq)(null, error);
        throw error;
    }
    return {
        seq, cmd, payload, reserved
    };
}
...
const listeners = {
    ...
    //"StatSvc.MsfConnProbe": function(){this._wt.ssoHeartbeat()},
    "trpc.msg.olpush.OlPushService.MsgPush": push.NTMsgListener,
    //"trpc.msg.register_proxy.RegisterProxy.InfoSyncPush": push.infoSyncPushListener,
    "trpc.qq_new_tech.status_svc.StatusService.KickNT": ntOfflineListener,
};
...
async function packetListener(pkt) {
    ...
    try {
        ...
        if (sso.seq && sso.seq < 0) {
            if (processedSeqs.has(sso.seq)) {
                this.logger.trace(`drop duplicate:${sso.cmd} seq:${sso.seq}`);
                return;
            }
            processedSeqs.add(sso.seq);
            setTimeout(() => {
                processedSeqs.delete(sso.seq);
            }, seqValidTime);
        }
        ...
        if (Reflect.has(listeners, sso.cmd))
            listeners[sso.cmd].call(this, sso.payload, sso.seq, sso);
