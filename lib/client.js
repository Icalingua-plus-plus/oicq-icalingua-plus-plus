...
const image = require("./message/image");
const { Gfs, getC2CFileUrlApi } = require("./message/file");
...
class Client extends EventEmitter {
    ...
    uid2uin_map  = new Map; //uid转uin
    uin2uid_map  = new Map; //uin转uid

    seq_id = 1145 + Math.round(Math.random() * 1419);
    handlers = new Map; //存放响应包的回调
    seq_cache = new Map; //一分钟内的缓存
    _registerRetryCount = 0; //register失败重试计数
    ...
    constructor(uin, config) {
        ...
        if (config.apk_info) {
            Object.assign(this.apk, config.apk_info);
        }
        if (this.config.useNT && this.apk.ssover < 20) this.apk.ssover = 20;
        this.logger = log4js.getLogger(`[${platforms[config.platform] ||
            ((config.platform % 2 === 1 ? "Android_" : "aPad_") + this.apk.ver)
            }:${uin}]`);
    ...
    makeForwardMsg(fakes, dm = false, target = 0, nt = true) {
    ...
    setEmojiReaction(group_id, msg_seq, id, type = 1, set = true) {
        return this._useProtocol(troop.setEmojiReaction, arguments);
    }
    sendButtonCallback(group_id, msg_seq, appid, id, data) {
        return this._useProtocol(troop.sendButtonCallback, arguments);
    }
    ...
    getNTPicURLbyFileid(fileid) {
        return this._useProtocol(image.getNTPicURLbyFileidApi, arguments);
    }

    getDevLoginInfo() {
        return this._useProtocol(nessy.getDevLoginInfo, arguments);
    }

    delDevLoginInfo(flag) {
        return this._useProtocol(nessy.delDevLoginInfo, arguments);
    }

    getC2CFileUrl(fileid) {
        return this._useProtocol(getC2CFileUrlApi, arguments);
    }
    ...
    saveStorageConfig() {
        try {
            fs.writeFileSync(path.join(this.dir, "storage.json"), JSON.stringify({
                sig_session: this.storage.sig_session.toString("base64"),
                session_key: this.storage.session_key.toString("base64"),
                ip: this.storage.ip,
    ...
    readStorageConfig() {
        try {
            const bakPath = path.join(this.dir, "storage.json");
            if (!fs.existsSync(bakPath))
                return;
            const bakJSON = fs.readFileSync(bakPath).toString();
            const bakConfig = JSON.parse(bakJSON);
            this.storage.sig_session = Buffer.from(bakConfig.sig_session, "base64");
            this.storage.session_key = Buffer.from(bakConfig.session_key, "base64");
            this.storage.ip = bakConfig.ip;
            this.storage.port = bakConfig.port;
        } catch (e) {
            this.logger.error(e);
        }
    }
