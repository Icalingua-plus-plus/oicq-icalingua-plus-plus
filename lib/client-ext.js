...
Client.prototype.getSign = async function(cmd, body, retry = 0) {
    ...
    const { data } = await axios.post(this.config.sign_api_addr,
        {
            ver: this.apk.ver,
    ...
    if (data.code !== 0) {
        if (retry < 3) {
            this.logger.warn(`[qsign] ${data.msg}(${data.code}), retry in 1s...`);
            await sleep(1000);
            return await this.getSign(cmd, body, retry + 1);
        }
        data.msg = data.msg || 'unknown error';
        throw new Error(`[qsign] ${data.msg}(${data.code})`);
    }
    if (!data.data.sign || !data.data.token || !data.data.extra) {
        throw new Error(`[qsign] server return invalid response: ${JSON.stringify(data)}`);
    }
