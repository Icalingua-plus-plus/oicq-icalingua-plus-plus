...
*/
Client.prototype._buildUniPacket = async function (cmd, body, seq = 0) {
    seq = seq ? seq : this.nextSeq();
    const startTime = Date.now();
    const signProtobuf = this.needSignCmd.includes(cmd) ? await this.getSign(cmd, Buffer.from(body), seq) : BUF0;
 ...
 * 消息同步
 */
Client.prototype.pbGetMsg = async function (force = false) {
