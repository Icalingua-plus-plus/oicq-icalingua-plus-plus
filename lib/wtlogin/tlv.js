...
const tlv_map = {
    ...
    0x544: function (code = "810_9") {
        //if (this.sig.t544[code]) return new Writer().writeBytes(this.sig.t544[code]);
        if (this.c.config.platform <= 5) {
            const salt = new Writer()
            const subCmd = parseInt(code.split("_")[1] || 9)
            if (["810_9", "810_a", "810_d", "810_f"]) {
                salt.writeU32(0)
            ...
            const t544 = sign((new Date()).getTime(), salt.read());
            this.logger.trace("salt", salt.read().toString("hex"));
            this.logger.trace("tlv544", t544);
