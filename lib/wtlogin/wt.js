...
class WtLogin {
    ...
    async sendLogin(cmd, body) {
        const pkt = await this._buildLoginPacket(cmd, this._buildOICQPacket(body), 2);
        try {
    ...
    async heartbeat() {
        const pkt = await this._buildLoginPacket("Heartbeat.Alive", BUF0, 0);
        return this.c.send(pkt).catch(NOOP);
    }

    async syncTimeDiff() {
        const pkt = await this._buildLoginPacket("Client.CorrectTime", BUF4, 0);
        this.c.send(pkt).then(buf => {
            try {
    ...
    async register(logout = false) {
        ...
        const pkt = await this._buildLoginPacket("StatSvc.register", body, 1);
        try {
     ...
     */
    async _buildLoginPacket(cmd, body, type) {
        this.c.nextSeq();
