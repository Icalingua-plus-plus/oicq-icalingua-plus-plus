...
class WtLogin {
    ...
    decodeT119(data, token = false) {
        const reader = new Reader(tea.decrypt(data, this.device.tgtgt), { objectMode: false });
        reader.read(2);
        ...
        this.sig = {
            ...
            st_sig: t[0x114] ? t[0x114] : this.sig.st_sig,
        ...
        if (this.sig.t543.length > 6 && !this.uid) {
            const t543 = pb.decode(this.sig.t543);
            try {
                this.uid = String(t543[9][11][1] || "");
            } catch { }
        }
        if (t[0x550] && !this.uid) {
    ...
    readT11A(data) {
        if (!data) return;
        const stream = new Reader(data, { objectMode: false });
        stream.read(2);
        ...
        switch (typ) {
            ...
            case 2:
                ...
                if (tgt.length === 32) {
                    ...
                    src.copy(buf, 128 - src.length);
                    let hash = createHash("sha256").update(buf).digest();
                    while (Buffer.compare(hash, tgt) !== 0) {
                        for (let i = 127; i >= 0; i--) {
                            buf[i] = (buf[i] + 1) & 0xFF;
                            if (buf[i] !== 0) break;
                        }
                        hash = createHash("sha256").update(buf).digest();
                        cnt++;
                        if (cnt > 1000000) {
    ...
    _decodeLoginResponse(payload) {
        ...
        if (type === 237) {
            ...
            const info = proto?.[60];
            if (info) {
                const title = String(info[7] || "登录失败");
                const content = String(info[8] || "未知错误，错误码：237");
                const message = `[${title}]${content}`;
                if (info[6] && info[6][3]) {
                    const jump = {
                        word: String(info[6][1]),
                        url: String(info[6][3]),
                    };
                    this.logger.mark(message);
                    this.logger.mark("访问URL完成验证后调用login()可直接登录（需要提交设备信息）。");
                    this.logger.mark(`${jump.word}: ${jump.url}`);
                    return this.c.em("system.login.auth", {
                        url: jump.url,
                        device: this.buildLoginAuthDevice(),
                    });
                }
            }
        }
        ...
        if (this.token_flag) {
            ...
            if (t[0x146]) {
                const stream = new Reader(t[0x146]);
            ...
            if (this.t10c) {
                // emp15 失败，可能 t106/t10c 失效，尝试用 emp11
                this.t10c = undefined;
                return fs.promises.rename(
                    path.join(this.dir, "t10c"),
                    path.join(this.dir, "t10c." + Date.now())
                ).catch(NOOP).then(this.passwordLogin.bind(this));
            }
        ...
        if (t[0x149]) {
            const stream = new Reader(t[0x149]);
            stream.read(2);
            const title = stream.read(stream.read(2).readUInt16BE()).toString();
            const content = stream.read(stream.read(2).readUInt16BE()).toString();
            const message = `[${title}]${content}`;
    ...
    async ssoHeartbeat() {
        ...
        const pkt = await this.c.sendUni("trpc.qq_new_tech.status_svc.StatusService.SsoHeartBeat", body).catch(NOOP);
    ...
    async register(logout = false) {
        ...
        try {
            ...
            const result = !rsp[2];
            if (!result && !logout) {
                this.logger.error(`上线失败：${rsp[3]}(${rsp[2]})`);
                await this.deleteToken();
            }
            return result;
        } catch {
    ...
    async registerNT(logout = false) {
        ...
        if (logout) {
            ...
            try {
                ...
                this.logger.error(`下线异常：${e.message}`);
                return false;
            }
        }
        ...
        const pb_buf = pb.encode({
            ...
            9: {
                ...
                2: 0, // kickPc
                3: String(this.apk.version.split(".")[3] || "14110"), // buildVer
                4: 0, // first
                5: 2052,
                6: {
                    1: d.brand + '-' + d.model,
        ...
        try {
            ...
            if (!result) {
                this.logger.error(`上线失败：${rsp[7] ? String(rsp[7][2]) : "未知错误"}`);
                await this.deleteToken();
            }
            return result;
        } catch (e) {
            this.logger.error(`上线异常：${e.message}`);
            return false;
        }
    }
     ...
     */
    async _buildLoginPacket(cmd, body, type, seq = 0, uin = this.uin) {
        ...
        if (ntRegCmds.includes(cmd)) {
            options = { nt_core: true, trace_parent: true };
        }
        const startTime = Date.now();
        let reserveFields = await this.c.buildSSOReserveField(cmd, Buffer.from(body), seq, options);
