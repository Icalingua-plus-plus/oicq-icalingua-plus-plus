/**
 * Tlv 544 (0C05) 算法
 * https://github.com/takayama-lily/oicq/pull/518
 */
"use strict";
...
var state = /** @class */ (function () {
    ...
    state.prototype.init = function (a, b, c, d) {
        this.nr = d;
        this.p = 0;
        initState(this, a, b, c);
    };
    state.prototype.encrypt = function (data) {
        var bp = 0;
        var dataLen = data.length & 0xffffffff;
        var _loop_1 = function () {
            if (this_1.p === 0) {
                refreshState(this_1);
            }
            var sb = Buffer.allocUnsafe(64);
            this_1.state.forEach(function (v, i) {
                sb.writeUint32LE(v, i * 4);
            });
            while (this_1.p != 64 && dataLen != 0) {
                data[bp] ^= sb[this_1.p];
                this_1.p++;
                bp++;
                dataLen--;
...
function sub_a(a, b) {
    var byteB = toBytes(b);
    var seq = [3, 2, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12];
...
function sub_b(a, b) {
    var byteB = toBytes(b);
    var seq = [3, 6, 9, 12, 7, 10, 13, 0, 11, 14, 1, 4, 15, 2, 5, 8];
    seq.forEach(function (v, i) {
        a[i] = a[i] ^ byteB[seq[i]];
    });
}
...
function sub_e(DI, p) {
    for (var i = 4; i > 0; i--) {
        var _a = [p[4 * i - 4], p[4 * i - 3], p[4 * i - 2], p[4 * i - 1]], a = _a[0], b = _a[1], c = _a[2], d = _a[3];
...
function sub_ab(a, b) {
    var ia = ((b >>> 28) << 4) + ((b >>> 24) & 15);
    var AX = a[Math.floor(ia / 16)][ia % 16] & 0xffffffff;
    var ib = b & 0xff;
    var BX = a[Math.floor(ib / 16)][ib % 16] & 0xffffffff;
    var ic = ((b >>> 8) & 15) + ((b >>> 8) & 240);
    var CX = a[Math.floor(ic / 16)][ic % 16] & 0xffffffff;
    var id = ((b >>> 16) & 15) + ((b >>> 16) & 240);
    var DX = a[Math.floor(id / 16)][id % 16] & 0xffffffff;
...
function sub_aa(a, b, c, d) {
    var dx = ((a & 15) << 9) + ((d[a] & 15) << 4);
    var di = (c[a & 15] >>> 4) & 15;
    var ax = (((d[a] >>> 4) & 15) << 4) + ((a & 15) << 9) + di;
    var res = b[Math.floor(ax / 512)][Math.floor(ax % 512 / 256)][Math.floor(ax % 512 % 256 / 16)][ax % 512 % 256 % 16 % 16];
    res = res << 4;
    dx = dx + (c[a & 15] & 15) + 256;
...
function transformInner(a, b) {
    var si = 0;
    for (var cnt = 1; cnt < 43;) {
        var ai = (((cnt - 1) & 31) << 4) + (a[si] >>> 4);
        var bi = ((cnt & 31) << 4) + (a[si] & 15);
 ...
 * @param curr
 * @param input
 * @param platform andeoid=1 ios=2 other=0
 */
function sign(curr, input, platform = 1) {
    seed = curr;
    var crcData = Buffer.allocUnsafe(21);
    curr = curr % 1000000;
    crcData.writeUInt32BE(curr & 0xffffffff, 0);
    input = Buffer.concat([input, crcData.slice(0, 4)]);
    var kt = Buffer.alloc(40);
    for (var i = 0; i < 2; i++) {
        kt[i] = keyTable[seededRandInt() % 50].charCodeAt(0) + 50;
    }
    kt[2] = kt[1] + 20;
    kt[3] = kt[2] + 20;
    key1.copy(kt, 6, 0, 4);
    ...
    var result = hash.update(input).digest();
    crcData[2] = platform;
    crcData[3] = crcData[4] = 0;
    kt.copy(crcData, 5, 0, 4);
    crcData.writeUInt32BE(curr, 9);
    result.copy(crcData, 13, 0, 8);
    var calcCrc = tencentCrc32(tab, crcData.slice(2));
    kt.writeUint32LE(calcCrc, 36);
    crcData[0] = kt[36];
    crcData[1] = kt[39];
    var nonce = seededRandInt() ^ seededRandInt() ^ seededRandInt();
    kt.writeUInt32BE(nonce, 0);
    kt.copy(kt, 4, 0, 4);
    kt.copy(kt, 8, 0, 8);
    transformInner(crcData, ts.encode);
    var encrypted = tencentEncryptB(kt.slice(0, 16), crcData);
    ...
    kt[31] = table2[seededRandInt() % 50].charCodeAt(0);
    kt[32] = table2[seededRandInt() % 50].charCodeAt(0);
    var addition = seededRandInt() % 9;
    while ((addition & 1) == 0) {
        addition = seededRandInt() % 9;
    }
...
function tencentEncryptionA(input, key, data) {
    var s = new state(new Uint32Array(16), new Uint32Array(16), 0, 0);
...
function tencentEncryptB(c, m) {
    ...
    var w = sub_f(encrypt.tableE, encrypt.tableF, encrypt.tableB);
    var out = Buffer.allocUnsafe(21);
    for (var i = 0; i < 21; i++) {
        if ((i & 0xf) == 0) {
            c.copy(buf);
