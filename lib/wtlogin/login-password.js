...
WtLogin.prototype.passwordLogin = async function () {
    try {
        const qimei = JSON.parse(String(await fs.promises.readFile(path.join(this.dir, "qimei.json"))));
        if (qimei.q16 && qimei.q36 && qimei.android_id === this.device.android_id) {
            this.device.qimei16 = qimei.q16;
            this.device.qimei36 = qimei.q36;
            this.logger.info("从缓存获取到QIMEI: " + this.device.qimei16 + " " + this.device.qimei36);
        } else {
            this.logger.warn("QIMEI 缓存缺失或无效，将重新获取。");
        }
    } catch { }
    if (this.apk.qua && (!this.device.qimei16 || !this.device.qimei36)) {
        ...
        this.device.qimei16 = qimei.q16;
        this.device.qimei36 = qimei.q36;
        fs.promises.writeFile(path.join(this.dir, "qimei.json"), JSON.stringify({
            q16: this.device.qimei16,
            q36: this.device.qimei36,
    ...
    try {
        await this.c.updateCmdWhiteList();
    } catch (e) {
        const message = e.message || "[qsign][CmdWhiteList] 未知错误";
        this.logger.error(message);
    }
    try {
        this.t106 = await fs.promises.readFile(path.join(this.dir, "t106"));
        try {
            this.t10c = await fs.promises.readFile(path.join(this.dir, "t10c"));
        } catch {
            this.logger.warn("你的登录缓存有缺失或者是旧版创建的，将使用旧版刷新包。");
        }
        const token = await fs.promises.readFile(path.join(this.dir, "token"));
        const stream = new Reader(token);
        const d2key = stream.readTlv();
        const d2 = stream.readTlv();
        const ticket = stream.readTlv();
        const sig = stream.readTlv();
        const srm = stream.readTlv();
        const tgt = stream.readTlv();
        this.sig.device_token = stream.readTlv();
        this.sig._ksid = stream.readTlv() || BUF0;
        if (d2key.length && d2.length && ticket.length && sig.length && srm.length && tgt.length) {
            this.sig.d2 = d2;
            this.sig.d2key = d2key;
    ...
    if (this.apk.nt && !this.c.config.forceWt && this.password_md5) {
        try {
            return await this.ntPasswordLogin(this.password_md5);
        } catch (e) {
            const message = e.message || "NT login failed";
            this.logger.error(message);
            return this.c.em("system.login.error", { code: -1, message });
        }
    }
    if (!this.password_md5)
        return this.fetchQrcode().catch((e) => {
...
WtLogin.prototype.sliderLogin = async function (ticket) {
    const [captchaTicket, randStr = ""] = String(ticket).trim().split(",", 2);
    if (this.ntHasCaptcha()) {
        return this.ntSubmitCaptcha(captchaTicket, randStr);
    }
    if (!this.t104)
        return this.logger.warn("未收到滑动验证码或已过期，你不能调用sliderLogin函数。");
    const t = this.tlvPacker;
    const tlvs = [
        t(0x193, captchaTicket),
        t(0x8),
        t(0x104),
        t(0x116),
    ];
...
WtLogin.prototype.tokenLogin = async function (d2) {
    if (this.t10c) return await this.tokenLoginWithT10C();
    const t = this.tlvPacker;
    const tlvs = [
        t(0x100),
        t(0x10a),
...
WtLogin.prototype.refreshD2 = async function (forceEmp11 = false) {
    if (!this.c.isOnline() || timestamp() - this.sig.emp_time < 21600)
        return
    if (this.t10c && !forceEmp11) return await this.refreshD2WithT10C();
    ...
    try {
        ...
        const stream = new Reader(payload);
        stream.read(2);
...
WtLogin.prototype.tokenLoginWithT10C = async function () {
...
WtLogin.prototype._emp_times = 0;
WtLogin.prototype.refreshD2WithT10C = async function () {
    if (++this._emp_times % 4 !== 0)
        return await this.exchangeEmp();
    this.device.tgtgt = this.t10c;
    const t = this.tlvPacker;
    const tlvs = [
        t(0x18),
