...
function highwayHttpUpload(readable, obj) {
    ...
    const MAX_QUEUED = 3  // 队列中最多保留的分块数，控制内存上限（3×512KB≈1.5MB）
    let seq = 1
    let offset = 0
    let readEnded = false

    // 顺序队列：每读到一个完整分块就推入，drain 循环逐个串行上传
    const queue = []
    let notify = null    // 用于队列非空时唤醒 drain
    let errFlag = null   // 记录首个错误，终止后续上传
    let quickDone = false

    const buildBuf = (chunk, chunkOffset) => {
        const head = pb.encode({
            1: {
                1: 1,
                2: String(this.uin),
        ...
        header.writeUInt8(40)
        header.writeUInt32BE(head.length, 1)
        header.writeUInt32BE(chunk.length, 5)
        return Buffer.concat([header, head, chunk, __])
    }

    // 读取流 → 分块 → 入队（带背压：队列满时暂停读取）
    readable.on("data", data => {
        if (errFlag || quickDone) return
        let _offset = 0
        while (_offset < data.length) {
            const chunk = data.slice(_offset, limit + _offset)
            _offset += chunk.length
            const buf = buildBuf(chunk, offset + _offset - chunk.length)
            queue.push({ buf, chunkLen: chunk.length })
            if (notify) { notify(); notify = null }
        }
        offset += data.length
        // 队列积压过多，暂停读取，等上传消化后再恢复
        if (queue.length >= MAX_QUEUED) {
            readable.pause()
        }
    })

    readable.on("err", err => { errFlag = err })

    // 串行 drain 循环：逐个从队列取分块上传
    const drain = async () => {
        let uploadedBytes = 0
        while (true) {
            while (queue.length === 0) {
                ...
                if (readEnded) {
                    // 流已结束且队列为空，检查是否全部完成
                    if (uploadedBytes >= obj.size) return
                }
                // 等待新分块入队
                await new Promise(r => { notify = r })
            }
            const { buf, chunkLen } = queue.shift()
            ...
            let rsp
            try {
                const respBuf = Buffer.from(r?.data)
                const header = respBuf.slice(9, respBuf.length - 1)
                rsp = pb.decode(header)
            } catch (err) {
                throw err
            }
            if (rsp?.[3] !== 0) {
                throw new Error(`${rsp[3]}, unknown highway error`)
            }
            uploadedBytes += chunkLen
            const percentage = (uploadedBytes / obj.size * 100).toFixed(2)
            this.logger.debug(`highway(http) chunk uploaded (${percentage}%)`)
            if (typeof obj.callback === "function")
                obj.callback(percentage)
            // 快传：服务端已持有全部数据，跳过后续分块
            if (rsp[7]?.toBuffer().length > 0) {
                quickDone = true
            ...
            // 队列消化后恢复读取
            if (queue.length < MAX_QUEUED && readable.isPaused()) {
                readable.resume()
            }
        }
    }
