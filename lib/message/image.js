...
async function getNTPicURLbyFileid(fileid) {
    ...
    if ([1406, 1407].includes(appidInFileId)) {
        const newRkey = appidInFileId === 1406 ? await getOffNTPicRkey.call(this) : await getGroupNTPicRkey.call(this);
        url = `https://gchat.qpic.cn/download?appid=${appidInFileId}&fileid=${fileid}${newRkey}&spec=0`;
    } else {
        // 异常 fileid，只能挨个尝试
        const groupNTPicRkey = await getGroupNTPicRkey.call(this);
        url = `https://gchat.qpic.cn/download?appid=1407&fileid=${fileid}${groupNTPicRkey}&spec=0`;
        if (!(await checkImgUrl(data.url))) {
            const offNTPicRkey = await getOffNTPicRkey.call(this);
...
module.exports = {
    ...
    getGroupNTPicRkey, getOffNTPicRkey,
    getNTPicURLbyFileid,
