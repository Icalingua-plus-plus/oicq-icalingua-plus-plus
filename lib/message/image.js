...
const rkey_type = {
    10: "offNTPicRkey",
    20: "groupNTPicRkey",
    34: "guildNTPicRkey", // 也许是频道图片，暂未发现使用场景
};

const rkey_cache = {
    offNTPicRkey: { value: '', expire: 0 },
    groupNTPicRkey: { value: '', expire: 0 },
    guildNTPicRkey: { value: '', expire: 0 },
}

/** @type {Promise<typeof rkey_cache> | null} */
let rkey_fetching = null;
...
class ImageBuilder {
    ...
    setNested() {
        ...
        if (this.c2c) {
            ...
            nested = {
                2: this.md5.toString("hex") + "." +  (this.as_face ? "gif" : (img_exts[this.type] || "jpg")),
                7: this.fid,
                8: 0,
                9: 0,
                10: 66,
    ...
    async download() {
        ...
        if (this.address.includes('&fileid=') && (this.address.startsWith('https://multimedia.nt.qq.com.cn/download') || this.address.startsWith('https://gchat.qpic.cn/download'))) {
            // 如果是QQNT图片，尝试获取rkey刷新URL
            try {
                const url = new URL(this.address);
                const fileid = url.searchParams.get('fileid');
                if (fileid) {
                    this.address = await getNTPicURLbyFileid.call(this.c, fileid);
                    this.c.logger.debug("下载QQNT网络图片: " + this.address);
                }
            } catch (e) {
                this.c.logger.warn(`获取QQNT图片rkey失败: ${e.message}`);
            }
        }
        try {
    ...
    async buildNested(cq) {
        let { file, cache, timeout, headers, type } = cq;
        this.as_face = type === "face";
 ...
 * @param {number} group_id 
 * @param {number} fileid 
 * @param {Buffer} fileMd5
 */
async function groupNTPicDown(group_id, fileid, fileMd5) {
    const req = {
        1: group_id,
        2: this.uin,
        3: fileid, //fileid
    ...
    const blob = await this.sendUni("ImgStore.GroupPicDown", body);
    const rsp = pb.decode(blob)[4];
    return "https://" + String(rsp[11]) + String(rsp[13]);
}
 ...
 * @this {import("../ref").Client}
 * @param {number} user_id 
 * @param {string} fileResId 
 */
async function offNTPicDown(user_id, fileResId) {
    const req = {
        1: user_id,
        2: this.uin,
        3: fileResId, //fileResId
        4: 4, //urlFlag
        6: 32, //urlType
        7: 5, //reqTerm
        8: 9, //reqPlatformType
    ...
    const blob = await this.sendUni("LongConn.OffPicDown", body);
    const rsp = pb.decode(blob)[3];
    return "https://gchat.qpic.cn" + String(rsp[11]);
}
 ...
 * @param {number|string} target 群号或UID
 * @param {boolean} c2c 是否为私聊
 */
async function getNTPicURL(imgInfo, target, c2c = false) {
    if (!target) target = c2c ? String(this.uid || this.uin) : 284840486;
    const body = pb.encode({
        1: {
            1: { 1: 1, 2: 200 },
            2: {
                101: 2, 102: 1,
                200: c2c ? 1 : 2,
                [c2c ? 201 : 202]: c2c ? { 1: 2, 2: String(target) } : { 1: target },
            },
            3: { 1: 2 },
        },
        3: {
    ...
    try {
        const rsp = await this.sendOidbSvcTrpcTcp(`OidbSvcTrpcTcp.0x11c${c2c ? 5 : 4}_200`, body);
        const rkey = rsp[3][1];
        if (!rkey)
            throw new Error("rkey不存在");
        return `https://${rsp[3][3][1]}${rsp[3][3][2]}${rkey}&spec=0`;
    } catch (e) {
        this.logger.warn("获取QQNT图片URL失败", e.message);
        return null;
    }
}
 ...
 * @this {import("../ref").Client}
 */
async function getNTPicRkey() {
    // 如果任一 rkey 缺失或已过期则需要刷新
    const now = common.timestamp();
    let needRefresh = false;
    for (const key of Object.keys(rkey_cache)) {
        const entry = rkey_cache[key];
        if (!entry.value || (entry.expire || 0) - now < 600) { // 提前 10 分钟刷新
            needRefresh = true;
            break;
        }
    }
    ...
    // 如果已有请求正在进行中，直接复用其结果
    if (rkey_fetching) {
        return rkey_fetching;
    }
    rkey_fetching = _fetchNTPicRkey.call(this).finally(() => {
        rkey_fetching = null;
    });
    return rkey_fetching;
}
 ...
 * @this {import("../ref").Client}
 */
async function _fetchNTPicRkey() {
    const body = pb.encode({
        1: {
            1: {
                1: 1,
    ...
    try {
        const rsp = await this.sendOidbSvcTrpcTcp("OidbSvcTrpcTcp.0x9067_202", body);
        const rkeys = rsp[4][1];
        if (!rkeys)
            throw new Error("rkey不存在");
        for (const v of rkeys) {
            rkey_cache[rkey_type[v[5]]].value = String(v[1]);
            rkey_cache[rkey_type[v[5]]].expire = (v[4] + v[2]) || common.timestamp() + 3000;
        }
    } catch (e) {
        this.logger.warn("获取QQNT Rkey失败", e.message);
    }
    return rkey_cache;
}
 ...
 * @param {string} fileid
 */
async function getNTPicURLbyFileid(fileid) {
    const appidInFileId = pb.decode(Buffer.from(fileid, "base64"))[4];
    const { offNTPicRkey, groupNTPicRkey, guildNTPicRkey } = await getNTPicRkey.call(this);
    let url = "";
    if ([1406, 1407].includes(appidInFileId)) {
        const newRkey = appidInFileId === 1406 ? offNTPicRkey : groupNTPicRkey;
        url = `https://gchat.qpic.cn/download?appid=${appidInFileId}&fileid=${fileid}${newRkey.value}&spec=0`;
    } else {
        // 异常 fileid，只能挨个尝试
        url = `https://gchat.qpic.cn/download?appid=1407&fileid=${fileid}${groupNTPicRkey.value}&spec=0`;
        if (!(await checkImgUrl(url))) {
            url = `https://gchat.qpic.cn/download?appid=1406&fileid=${fileid}${offNTPicRkey.value}&spec=0`;
            if (!(await checkImgUrl(url))) {
                url = `https://gchat.qpic.cn/download?appid=${appidInFileId}&fileid=${fileid}${guildNTPicRkey.value}&spec=0`;
            }
        }
    }
    return url;
...
async function getNTPicURLbyFileidApi(fileid) {
    const url = await getNTPicURLbyFileid.call(this, fileid);
    return { result: 0, data: url };
}

module.exports = {
    ImageBuilder, uploadImages, buildImageFileParam,
    groupNTPicDown, offNTPicDown, checkImgUrl,
    getNTPicURL,
    getNTPicURLbyFileid,
    getNTPicURLbyFileidApi,
};
