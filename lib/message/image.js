 ...
 * @param {String} uid
 */
async function getOffNTPicURL(imgInfo, uid) {
    ...
    try {
        const rsp = await this.sendOidbSvcTrpcTcp("OidbSvcTrpcTcp.0x11c5_200", body);
        const rkey = rsp[3][1];
        if (!rkey)
 ...
 * @param {number} gid
 */
async function getGroupNTPicURL(imgInfo, gid = 284840486) {
    const body = pb.encode({
        1: {
            1: {
                1: 1,
    ...
    try {
        const rsp = await this.sendOidbSvcTrpcTcp("OidbSvcTrpcTcp.0x11c4_200", body);
        const rkey = rsp[3][1];
        if (!rkey)
            throw new Error("rkey不存在");
        return `https://${rsp[3][3][1]}${rsp[3][3][2]}${rkey}&spec=0`;
...
async function getNTPicRkey() {
    ...
    for (const key of Object.keys(rkey_cache)) {
        ...
        if (!entry.value || entry.expire <= now) {
            needRefresh = true;
            break;
        }
    }
 ...
 * @this {import("../ref").Client}
 */
async function getGroupNTPicRkey() {
    const rsp = await getGroupNTPicURL.call(this, {
        2: "CNL-BhjClnU",
        3: 1,
    });
    if (!rsp)
 ...
 * @this {import("../ref").Client}
 */
async function getOffNTPicRkey() {
    const rsp = await getOffNTPicURL.call(this, {
        2: "CNL-BhjClnU",
        3: 1,
    });
    if (!rsp)
...
module.exports = {
    ...
    getGroupNTPicURL, getOffNTPicURL,
    getNTPicURLbyFileid,
