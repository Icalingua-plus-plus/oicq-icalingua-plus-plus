...
const { getC2CMsgs, getGroupMsgs, getLastSeq, getNTC2CMsgs } = require("./history");
...
const { c2cNTMsgConverter } = require("../onlinepush");
...
async function getOneMsg(message_id) {
    const ret = await getMsgs.call(this, message_id, 1, true);
...
/**
 * 获取从message_id(群聊未撤回时必包括自身，私聊不包括自身)往前的count条消息
 * @this {import("../ref").Client}
 * @param {string} message_id 
 * @param {number} count 
 * @param {boolean} get_one 
 * @returns {import("../ref").ProtocolResponse}
 */
async function getMsgs(message_id, count = 20, get_one = false) {
    ...
    if (message_id.length > 24) {
        ...
        let is_nt = true;
        try {
            msgs = await getNTC2CMsgs.call(this, user_id, time ? time : common.timestamp(), 20, get_one ? 0 : random);
        } catch (e) {
            this.logger.debug(e.message);
            is_nt = false;
            msgs = await getC2CMsgs.call(this, user_id, time ? time : common.timestamp(), 20, get_one ? 0 : random);
        }
        for (let i = msgs.length - 1; i >= 0; --i) {
            const msg = is_nt ? c2cNTMsgConverter.call(this, msgs[i]) : msgs[i];
            if (get_one && time && genRandom(msg[1][7]) !== random && !data.length)
                continue;
            try {
