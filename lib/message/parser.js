...
const { buildImageFileParam, groupNTPicDown, offNTPicDown, checkImgUrl, getOffNTPicURL, getGroupNTPicURL } = require("./image");
...
class Parser {
    ...
    async parsePartialElem(type, elem) {
        ...
        case 46: //markdown button
            break;
        case 48: //image of qqnt
            const imgData = await this.parseNTImgElem(type, elem);
            if (imgData) {
                msg.type = "image";
                msg.data = imgData;
                brief = "[图片]";
            ...
            if (type === 16) { //extraInfo 额外情报
                ...
                case 53: //commonElem
                    if (elem[1] === 3) { //flash
                        ...
                        await this.parsePartialElem(48, elem[2]);
     ...
     * @param {import("../ref").Proto} elem 
     */
    async parseNTImgElem(type, elem) {
        const data = { };
        if (!elem[2] || !elem[2][1]) return null;
        const dm = !!elem[2][1][11];
        const extra = dm ? elem[2][1][11] : elem[2][1][12];
