...
const { buildImageFileParam, groupNTPicDown, offNTPicDown, checkImgUrl, getOffNTPicURL, getGroupNTPicURL, getOffNTPicRkey, getGroupNTPicRkey } = require("./image");
const { Gfs, getC2CFileUrl } = require("./file");
const { getVideoUrl, getPttUrl } = require("./ptt");
...
class Parser {
    ...
    /**
     * 解析排他型消息节点
     * xml, json, ptt, video, flash, file, shake, poke, qlottie
     * @private
     * @param {number} type 
    ...
    async parsePartialElem(type, elem) {
        ...
        case 48: //ntv2richmedia
            const imgData = await this.parseNTImgElem(type, elem[2], elem[3]);
            if (imgData) {
                msg.type = "image";
                msg.data = imgData;
                brief = "[图片]";
            ...
            if (type === 16) { //extraInfo 额外情报
                ...
                case 53: //commonElem
                    if (elem[1] === 3) { //flash
                        ...
                        await this.parseExclusiveElem(37, elem);
                        ...
                        if (elem[2] && elem[2][1] && elem[2][1][2] && elem[2][1][2][1] && elem[2][1][2][3])
                            this.imgprefix = "https://" + String(elem[2][1][2][3]) + String(elem[2][1][2][1]);
                    } else if (elem[1] == 45) { //markdown
                        await this.parsePartialElem(type, elem[2]);
                    }
                    break;
                case 45: //reply
    ...
    async parseNTImgElem(type, elem, businessType = 0) {
        if (![0, 10, 20].includes(businessType)) return null;
        const data = { };
        if (!elem[2] || !elem[2][1]) return null;
        const dm = businessType ? businessType === 10 : !!elem[2][1][11];
        const extra = elem[2][1][11] || elem[2][1][12] || { 30: "" };
        const ntPath = String(extra[30] || "");

        const imgInfo = elem[1][1];
        const md5 = imgInfo[1][2]; //string, not buffer
        const sha1 = imgInfo[1][3];
        const fileName = imgInfo[1][4];
        const size = imgInfo[1][1];
        const width = imgInfo[1][6];
        const height = imgInfo[1][7];
        const imgType = imgInfo[1][5][2];

        const fileid = imgInfo[2];
        const storeId = imgInfo[3];
        const time = imgInfo[4];

        data.file = buildImageFileParam(md5.toString(), size, width, height, imgType);
        ...
        if (!(await checkImgUrl(data.url))) {
            const appidInURL = data.url.match(/appid=(\d+)/);
            if (appidInURL) {
                const newAppid = appidInURL[1] === "1406" ? "1407" : "1406";
                data.url = data.url.replace(appidInURL[0], `appid=${newAppid}`);
                const newRkey = newAppid === "1406" ? await getOffNTPicRkey.call(this.c) : await getGroupNTPicRkey.call(this.c);
                data.url = data.url.replace(/&rkey=[^&]+/, `${newRkey}`);
            }
        }
        return data;
    }
    ...
    async parseImgElem(type, elem) {
        ...
        if (this.imgprefix && ntPath && ntPath.startsWith("&rkey=")) {
            data.url = this.imgprefix + String(ntPath) + "&spec=0";
            this.imgprefix = null;
            if (await checkImgUrl(data.url))
                return data;
        }
        ...
        let ntFileId = null;
        if (dm) { //私图
            if (ntPath && ntPath.startsWith("/download") && ntPath.includes("rkey=")) {
            ...
            if (!(await checkImgUrl(data.url))) {
                ntFileId = data.url.match(/&fileid=([^&]+)/);
                // 图片地址失效，尝试发包获取
        ...
        if (ntFileId && !(await checkImgUrl(data.url))) {
            const offNTPicRkey = await getOffNTPicRkey.call(this.c);
            data.url = `https://gchat.qpic.cn/download?appid=1406&fileid=${ntFileId[1]}${offNTPicRkey}}&spec=0`;
            if (!(await checkImgUrl(data.url))) {
                const groupNTPicRkey = await getGroupNTPicRkey.call(this.c);
                data.url = `https://gchat.qpic.cn/download?appid=1407&fileid=${ntFileId[1]}${groupNTPicRkey}}&spec=0`;
            }
        }
        return data;
    }
    ...
    async parseReplyElem(elem) {
        ...
        try {
            ...
            let seq = elem[1], user_id = elem[2], replyed = Array.isArray(elem[5]) ? elem[5] : [elem[5]];
            if (this.gid) {
