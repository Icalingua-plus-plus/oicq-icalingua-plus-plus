...
const { buildImageFileParam, groupNTPicDown, offNTPicDown, checkImgUrl, getOffNTPicURL, getGroupNTPicURL, getOffNTPicRkey } = require("./image");
...
class Parser {
    ...
    async parseNTImgElem(type, elem, businessType = 0) {
        ...
        data.url = data.url.replace("multimedia.nt.qq.com.cn", "gchat.qpic.cn"); //这个host下面似乎不检查rkey有效期以及appid是否对应
    ...
    async parseImgElem(type, elem) {
        ...
        if (ntFileId && !(await checkImgUrl(data.url))) {
            ...
            if (!(await checkImgUrl(data.url))) {
                data.url = `https://gchat.qpic.cn/download?appid=1407&fileid=${ntFileId[1]}${offNTPicRkey}}&spec=0`;
            }
        }
        return data;
    }
