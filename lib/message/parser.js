...
const { buildImageFileParam, groupNTPicDown, offNTPicDown } = require("./image");
...
class Parser {
    ...
    async parseExclusiveElem(type, elem) {
        ...
        case 3: //flash
            msg.type = "flash";
            msg.data = await this.parseImgElem(elem);
            brief = "[闪照]";
            break;
        case 0: //ptt
            msg.type = "record";
        ...
        while (true) {
            ...
            if (type === 16) { //extraInfo 额外情报
                ...
                case 53: //commonElem
                    ...
                    } else if (elem[1] == 48) { //image prefix of qqnt
     ...
     * @param {import("../ref").Proto} elem 
     */
    async parseImgElem(elem) {
        const data = { };
        if (elem[29] && elem[29][30] && String(elem[29][30]).startsWith("&rkey=")) {
            //QQNT 私图
            if (this.imgprefix) {
                data.file = buildImageFileParam(elem[7].toHex(), elem[2], elem[9], elem[8], elem[5]);
                data.url = this.imgprefix + String(elem[29][30]) + "&spec=0";
                this.imgprefix = null;
                return data;
            }
        } else if (elem[34] && elem[34][30] && String(elem[34][30]).startsWith("&rkey=")) {
            //QQNT 群图
            if (this.imgprefix) {
                data.file = buildImageFileParam(elem[13].toHex(), elem[25], elem[22], elem[23], elem[20]);
                data.url = this.imgprefix + String(elem[34][30]) + "&spec=0";
                this.imgprefix = null;
                return data;
            }
        }
        if (!elem[7]) { //QQNT 群聊转发到私聊图
            data.file = buildImageFileParam(elem[13].toHex(), elem[25], elem[22], elem[23], elem[20]);
            data.url = await offNTPicDown.call(this.c, this.uid, elem[2]);
        } else if (elem[7].toHex) { //私图
            data.file = buildImageFileParam(elem[7].toHex(), elem[2], elem[9], elem[8], elem[5]);
            if (elem[15])
                data.url = "https://c2cpicdw.qpic.cn" + elem[15];
            else if (elem[10]) {
                if (elem[23])
                    data.url = await groupNTPicDown.call(this.c, this.gid, elem[23], elem[7]);
                else
                    data.url = await offNTPicDown.call(this.c, this.uid, elem[10]);
            }
            /* deprecated
            if (!data.url || !data.url.includes("offpic_new")) {
                const keys = Object.keys(elem).reverse();
                for (let key of keys) {
                    if (elem[key].toString || elem[key].toString().length !== 16) {
                        data.url = await offNTPicDown.call(this.c, this.uid, String(elem[key]));
                        break;
                    }
                }
            }
            */
            ...
            if (elem[16])
                ...
                data.url = `https://gchat.qpic.cn/gchatpic_new/${this.uid}/${code2uin(this.gid)||0}-${elem[7]>>>0}-${elem[13].toHex().toUpperCase()}/0?term=2`;
            if (!data.url.includes("gchatpic_new"))
                data.url = await groupNTPicDown.call(this.c, this.gid, elem[7], elem[13]);
        }
        return data;
    }
