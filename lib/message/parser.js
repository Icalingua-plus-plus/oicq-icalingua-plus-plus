...
class Parser {
    ...
    async parseExclusiveElem(type, elem) {
        ...
        case 37: //qlottie
                ...
                if (elem[2][2]) msg.data.extra = JSON.stringify({
                    packId: elem[2][1], //string
                    lottieId: elem[2][2], //string
                    faceId: elem[2][3], //number
                    sourceType: elem[2][4], //number
                    lottieType: elem[2][5], //number
                    resultId: elem[2][6], //string
                    text: elem[2][7], //string
                    surpriseId: elem[2][8], //string
    ...
    async parseImgElem(elem) {
        ...
        if (!elem[7]) { //QQNT私图
            ...
            data.url = await offNTPicDown.call(this.c, this.uid, elem);
        } else if (elem[7].toHex) { //私图
            data.file = buildImageFileParam(elem[7].toHex(), elem[2], elem[9], elem[8], elem[5]);
            if (elem[29] && elem[29][30])
                data.url = "https://gchat.qpic.cn" + elem[29][30] + "&spec=0&rf=naio";
            else if (elem[15])
