...
const { buildImageFileParam, groupNTPicDown, offNTPicDown, checkImgUrl, getNTPicURL, getNTPicURLbyFileid } = require("./image");
...
const { uid2uin } = require("../core/friendlist");
 ...
 * @returns {Promise<Buffer>}
 */
async function _downloadMultiMsgNt(resid, bu) {
    const body = pb.encode({
        1: {
            1: {
                2: this.uin,
    ...
    const payload = await this.sendUni("trpc.group.long_msg_interface.MsgService.SsoRecvLongMsg", body);
    try {
...
class Parser {
    ...
    async parsePartialElem(type, elem) {
        ...
        case 46: //markdown button
            ...
            try {
                const body = elem[1];
                const appid = body[2];
                const rows = Array.isArray(body[1]) ? body[1] : [body[1]];
                msg.data = {
                    appid: appid,
                    rows: [],
                };
                for (const row of rows) {
                    const cols = Array.isArray(row[1]) ? row[1] : [row[1]];
                    const buttons = [];
                    for (const col of cols) {
                        ...
                        if (col[1]) btn.id = String(col[1]);
                        if (col[2]) {
                            btn.render_data.label = String(col[2][1] || "");
                            btn.render_data.visited_label = String(col[2][2] || "");
                            btn.render_data.style = Number(col[2][3]) || 0;
                        }
                        if (col[3]) {
                            btn.action.type = Number(col[3][1]) || 0;
                            btn.action.unsupport_tips = String(col[3][4] || "");
                            btn.action.data = String(col[3][5] || "");
                            btn.action.reply = col[3][7] === 1;
                            btn.action.enter = col[3][8] === 1;
                            if (col[3][2]) {
                                btn.action.permission.type = Number(col[3][2][1]) || 0;
                                btn.action.permission.specify_role_ids = col[3][2][2] || [];
                                btn.action.permission.specify_user_ids = col[3][2][3] || [];
                            }
                        }
                        buttons.push(btn);
                    }
                    msg.data.rows.push({ buttons });
                }
            } catch (e) {
                this.c.logger.error("解析markdown button失败", e);
            }
            break;
    ...
    async parseElems(elems) {
        ...
        while (true) {
            ...
            if (type === 16) { //extraInfo 额外情报
                ...
                if (elem[6] === 1 && elem[7]) {
                    try {
                        const buf = await _downloadMultiMsgNt.call(this.c, elem[7].toBuffer(), 1);
                ...
                case 53: //commonElem
                    ...
                    } else if (elem[1] == 46) { //markdown button
                        await this.parsePartialElem(46, elem[2]);
                    }
                    break;
                case 45: //reply
    ...
    async parseReplyElem(elem) {
        ...
        try {
            ...
            let seq = elem[1], user_id = elem[2], user_uid = elem[8] ? elem[8][6] : '';
            if (!user_id && user_uid) user_id = await uid2uin.call(this.c, user_uid); // 某些第三方实现不塞uin只塞uid
            ...
            if (original) {
                try {
                    if (this.gid) {
                        const parser = await Parser.invoke(this.c, user_id, this.gid, original[3][1]);
                        msg.data.message = parser.message;
                    } else {
                        const parser = await Parser.invoke(this.c, user_id, 0, original[3][1]);
                        msg.data.message = parser.message;
                    }
                } catch (e) {
                    this.c.logger.error("解析回复原消息失败", e);
                }
            }
...
async function parseForwardMsg(resid, fileName) {
    const data = [];
    const blob = await _downloadMultiMsgNt.call(this, String(resid), 2);
    /**
     * @type {import("../ref").Msg[]}
     */
    let msgs = pb.decode(blob)[2], flag = false;
    ...
    if (!flag) throw new Error("wrong fileName");
    if (Array.isArray(msgs))
        msgs = msgs[0];
    msgs = msgs[2][1];
    if (!Array.isArray(msgs))
    ...
    for (let msg of msgs) {
        ...
        let head_img = "";
        if (head[8]) { // group
            nickname = String(head[8][4]);
            try {
                group_id = head[8][1];
            } catch { }
        } else { // c2c
            try {
                nickname = String(head[7][6]);
                group_id = head[8][1];
            } catch { }
        }
        try {
            head_img = String(content[15][5]);
        } catch { }
        try {
            let parser;
            if (content[1] === 529 && content[2] === 4 && body[2]) { // 合并转发的文件
                const file_msg = body[2];
                try {
                    if (String(file_msg[6][2][4])[0] === "/") { // 合并转发目的地是群
                ...
                parser = parser ?? await _parseC2CFileElem.call(this, file_msg[1]); // 合并转发目的地是私聊
            } else {
                parser = await Parser.invoke(this, user_id, group_id, body[1]); // 普通消息
            }
            data.push({
