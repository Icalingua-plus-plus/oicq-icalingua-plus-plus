...
const { buildImageFileParam, groupNTPicDown, offNTPicDown, checkImgUrl } = require("./image");
...
class Parser {
    ...
    async parseNTImgElem(type, elem) {
        ...
        const imgInfo = elem[1][1][1];
        const md5 = imgInfo[2];
        const sha1 = imgInfo[3];
        const fileName = imgInfo[4];
        const size = imgInfo[1];
        const width = imgInfo[6];
        const height = imgInfo[7];
        const imgType = imgInfo[5][2];

        const fileid = elem[1][1][2];
        const storeId = elem[1][1][3];
