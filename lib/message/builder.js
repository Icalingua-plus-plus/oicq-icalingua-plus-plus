...
const { makePttElem, makeVideoElem } = require("./ptt");
const { ImageBuilder, uploadImages } = require("./image");
const pb = require("../algo/pb");
const common = require("../common");
const { uploadMultiMsg } = require("./multi");
const { parseC2CMessageId, parseGroupMessageId, genMessageUuid, genC2CMessageId, genRandom, escapeXml } = common;
...
class Builder {
     ...
     */
    constructor(c, target, type, file = null) {
        this.c = c;
        this.target = target;
        this.type = type;
        this.file = file;
     ...
     * @param {string} text 
     * @param {Buffer} attr6 
     * @param {Buffer} extra12
     */
    buildTextElem(text, attr6 = null, extra12 = null) {
        text = String(text);
        let n = 0;
        while (n < text.length) {
            let m = n + 80;
    ...
    buildAtElem(cq) {
        ...
        try {
            ...
            uid = this.c.uin2uid_map.get(q) || uid;
        }
        const extra12 = {
            3: flag === 1 ? 1 : 2,
            4: q,
        ...
        this.buildTextElem(display, attr6, extra12);
    }
    ...
    buildFaceElem(cq) {
        ...
        if (cq.qlottie) {
            ...
            if (cq.extra) {
                try {
                    const extra = JSON.parse(cq.extra);
                    Object.assign(lottieElem, extra);
                } catch { }
            }
            this.elems2.push([
                {
                    53: {
                        ...
                        2: {
                            1: lottieElem.packId,
                            2: lottieElem.lottieId, //QQ只靠小表情的faceId来展示对应lottie表情
                            3: lottieElem.faceId,
                            4: lottieElem.sourceType,
                            5: lottieElem.lottieType, //展示形式，1:普通 2:有结果 3:接龙lottie表情
                            6: lottieElem.resultId, //有结果时的展示的结果动画id
                            7: lottieElem.text,
                            8: lottieElem.surpriseId, //全屏动画id
                            9: lottieElem.ramdomType,
                            10: lottieElem.chainCount, //接龙次数
                        },
                        3: lottieElem.businessType //1:普通 2:服务器随机resultId 3:接龙lottie表情
                    }
                },
                {
                    1: {
                        1: lottieElem.text,
                        12: {
                            1: "[" + lottieElem.text.replace("/", "") + "]请使用最新版手机QQ体验新功能"
                        }
                    }
                }
            ]);
    ...
    async buildImageElem(cq) {
        ...
        try {
            ...
            return this.c.logger.warn(e.message);
        }
    ...
    async buildPttElem(cq) {
        try {
            const elem = await makePttElem.call(this.c, this.type == 1 ? this.target : 1, cq);
            const el = pb.decode(elem);
     ...
     */
    async buildVideoElem(cq) {
        let file = String(cq.file);
        let buf;
        if (file.startsWith("protobuf://")) {
            buf = Buffer.from(file.replace("protobuf://", ""), "base64");
        } else {
            try {
                buf = await makeVideoElem.call(this.c, this.target, file)
            } catch (e) {
                return this.c.logger.warn(e.message);
            }
        }
    ...
    async buildLocationElem(cq) {
        ...
        await this.buildJsonElem(obj, "收到[[应用]地图]消息，请升级QQ版本查看");
    }
    ...
    async buildShareElem(cq) {
        ...
        await this.buildXmlElem(xml, 1, url);
    }
     ...
     * @param {string} text 
     */
    async buildXmlElem(xml, svcid = 60, text = "") {
    ...
    async buildReplyElem(cq) {
        if (this.reply)
            return;
        try {
            let user_id, seq, random, time, flag, source;
            try {
                const _message = await this.getMsg(cq.id);
                user_id = _message.user_id;
                seq = _message.seq;
                random = _message.random;
                time = _message.time;
                flag = _message.flag;
                source = _message.msg[3][1][2];
            } catch {
                const parsed = Buffer.from(cq.id, "base64");
                if (cq.id.length > 24) {
                    // Group
                    ...
                    flag = parsed.length >= 17 ? parsed.readUint8(16) : 0;
                }
                source = this.getTextElem(cq.text || " ");
            }
            if (Array.isArray(source)) {
                const bufs = [];
                for (let v of source)
                    bufs.push(v.toBuffer());
                source = bufs;
            } else if (source.toBuffer) {
                source = source.toBuffer();
            }
    ...
    /**
     * @private
     * @param {import("../ref").MiraiElem["data"]} cq 
     */
    buildMiraiElem(cq) {
        const { data } = cq;
        this.elems.push({
    ...
    async getMsg(id) {
        if (id.length > 24) {
            ...
            const msgs = await getC2CMsgs.call(this.c, user_id, time, 10);
            for (let i = msgs.length - 1; i >= 0; --i) {
                const msg = msgs[i];
                if (genRandom(msg[1][7]) === random)
    ...
    async buildElem(type, data) {
        ...
        case "record":
            ...
            break;
        case "video":
            await this.buildVideoElem(data);
            break;
        case "location":
            await this.buildLocationElem(data);
            break;
        case "music":
            await this.buildMusicElem(data);
            break;
        case "share":
            await this.buildShareElem(data);
            break;
        case "anonymous":
        ...
        case "json":
            if (data.data)
                await this.buildJsonElem(data.data, data.text);
            break;
        case "xml":
            if (data.data)
                await this.buildXmlElem(data.data, data.type, data.text);
            break;
        case "node":
            this.buildNodeElem(data);
            break;
        case "mirai":
            this.buildMiraiElem(data);
            break;
        case "markdown":
            this.c.logger.warn("暂不支持发送markdown消息");
            break;
        case "raw":
            try {
                this.elems.push(...JSON.parse(data.data));
            } catch {
                this.c.logger.warn("发送的原始消息元素数据有误");
            }
            break;
        default:
    ...
    async buildAndSend(message, escape) {
        ...
        if (!this.elems.length && this.file === null) {
            if (rsp) return rsp;
    ...
    async setRouting() {
        ...
        if (this.type > 0) {
            ...
            let uid;
            if (this.c.config.useNT) uid = this.c.uin2uid_map.get(user_id);
            routing = { 1: { 1: user_id, 2: uid } };
            if (this.file !== null) {
                routing = { 15: { 1: user_id, 2: 4, 8: uid } };
            }
    ...
    buildPbSendMsgPkt(rich, content = PB_CONTENT, random = undefined) {
        ...
        return pb.encode({
            1: this.routing,
            2: content,
            3: this.file !== null ? this.file : { 1: rich },
            4: this.seq,
            5: this.random,
    ...
    async send(rich, flag = false) {
        ...
        if (retcode === 0) {
            if (this.type === 0) { //私聊
                if (rsp[14] === 0) {
                    let emsg = rsp[2] ? String(rsp[2]) : "";
                    this.c.logger.error(`send failed: [${TYPES[this.type]}: ${this.target}] ${emsg}`);
                    return { result: -1, emsg };
                }
                message_id = genC2CMessageId(this.target, this.seq, this.random, rsp[3], 1);
            }
            if (this.type === 1 && !message_id) { //群聊
                message_id = await this.waitForMessageId((flag && this.c.config.resend) ? (this.length <= 80 ? 2000 : 500) : 5000);
                if (!message_id) {
