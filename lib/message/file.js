...
class Gfs {
    ...
    async download(fid) {
        ...
        return {
            name: file.name,
            url: encodeURI(`https://${rsp[5]}/ftn_handler/${rsp[6].toHex()}/?fname=${file.name}`),
            size: file.size,
            md5: file.md5,
...
async function getC2CFileUrl(fileid) {
    ...
    const host = String(rsp[90] || rsp[110] || "");
    const _url = new URL(url);
    if (host && host.endsWith(".ftn.qq.com")) {
        _url.hostname = host;
        _url.protocol = "https";
    }
 ...
 * @param {Buffer|string} fileid 
 */
async function getC2CFileUrlApi(fileid) {
 ...
 * @param {Buffer|string} fileid 
 */
async function getSyncDeviceFileUrl(fileid) {
    const body = pb.encode({
        1: 1200,
        14: {
            10: this.uin,
...
module.exports = {
    getC2CFileUrl, getC2CFileUrlApi, getSyncDeviceFileUrl, Gfs
};
