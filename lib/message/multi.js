...
async function buildXmlElem(xml, svcid = 35) {
...
async function _downloadMultiMsgNt(resid, bu) {
    ...
    try {
        ...
        return common.unzip(rsp[4].toBuffer());
    } catch (e) {
        e.message = "wrong resid";
        throw e;
    }
 ...
 * @param {number} retry
 */
async function makeForwardMsg(iterable, dm = false, target = 0, nt = true, retry = 0) {
    ...
    for (const fake of iterable) {
        ...
        let brief = "";
        {
            ...
            for (const elem of sendable) {
                ...
                case "image":
                    ...
                    break;
                case "reply":
                    let user_id, seq, _random, time, source, group_id;
                    const parsed = Buffer.from(elem.data.id, "base64");
                    if (elem.data.id.length > 24) {
                        // Group
                        group_id = parsed.readUInt32BE(0);
                ...
                case "xml":
                    ...
                    if (resIdRegex.test(elem.data.data) && fileNameRegex.test(elem.data.data)) {
                        ...
                        const buf = await (nt ? _downloadMultiMsgNt : _downloadMultiMsg).call(this, resId, 2);
        ...
        if (fake.bubble_id && fake.bubble_id !== -1) {
            elements.push({
                9: {
                    1: fake.bubble_id
                }
        ...
        if (!nt) {
            ...
            nodes.push({
                1: {
                    ...
                    //2: this.uid,
                    5: dm ? this.uin : null,
                    6: dm ? this.uid || null : null,
                    7: dm ? { 6: fake.nickname } : null,
                    8: dm ? null : { 1: target || this.uin, 4: fake.nickname },
                },
                2: {
                    1: dm ? 166 : 82,
                    2: dm ? 11 : null,
    ...
    try {
        var resid = await (nt ? uploadMultiMsgNt : uploadMultiMsg).call(this, target, compressed, dm);
    } catch (e) {
        if (String(e.message).includes("-10122")) {
            // 设备类型不支持nt
            this.logger.warn("failed to upload forward msg with NT protocol, try old protocol -> " + e.message);
            return makeForwardMsg.call(this, iterable, dm, target, false);
        }
        if (retry < 3) {
            this.logger.warn("failed to upload forward msg, retry -> " + e.message);
            await sleep(1000);
            return makeForwardMsg.call(this, iterable, dm, target, nt, retry + 1);
        }
        throw new Error("failed to upload forward msg -> " + e.message);
    }
 ...
 * @param {boolean} dm 是否为私聊消息
 * @returns {Promise<Buffer>} resid
 */
async function uploadMultiMsg(target, compressed, dm = false) {
    const body = pb.encode({
        1: 1,
        2: 5,
        3: 9,
 ...
 * @returns {Promise<Buffer>} resid
 */
async function uploadMultiMsgNt(target, compressed, dm = false) {
    const body = pb.encode({
        2: {
            1: dm ? 1 : 3,
            2: {
    ...
    const payload = await this.sendUni("trpc.group.long_msg_interface.MsgService.SsoSendLongMsg", body);
    const rsp = pb.decode(payload)[2];
    return rsp[3].toString();
}

module.exports = {
    makeForwardMsg, uploadMultiMsg, uploadMultiMsgNt
};
