            ...
            try {
                ...
                if (fake.consistent) {
                    if (fake.id.length > 24) {
                        // Group
                        nodes[p][1][5] = parsed.readUInt32BE(8);
                        const _random = parsed.readUInt32BE(12);
                        nodes[p][1][20][2] = _random;
                        nodes[p][1][7] = common.genMessageUuid(_random);
                    } else {
                        // C2C
                        nodes[p][1][5] = parsed.readUInt32BE(4);
