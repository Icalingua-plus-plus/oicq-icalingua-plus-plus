        ...
        let brief = "";
        {
            ...
            for (const elem of sendable) {
                ...
                case "xml":
                    ...
                    if (resIdRegex.test(elem.data.xml) && fileNameRegex.test(elem.data.xml)) {
                        const resId = resIdRegex.exec(elem.data.xml)[1];
                        const fileName = fileNameRegex.exec(elem.data.xml)[1];
                        const buf = await _downloadMultiMsg.call(this, resId, 2);
                        const msgs = pb.decode(buf)[2];
                        if (!msgs.isArray()) {
                            msgs = [msgs];
                        }
                        for (let msg of msgs) {
