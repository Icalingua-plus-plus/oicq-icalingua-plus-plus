...
class Network extends net.Socket {
    ...
    async _queryServerList() {
        ...
        const HttpServerListReq = jce.encodeStruct([
            null,
            0, 0, 1, "00000", 100, this.c.apk.subid, this.c.device.android_id, 0, 0, 0,
            0, 0, 0, 1
        ]);
        const extra = {
            service: "ConfigHttp",
        ...
        return await new Promise((resolve, reject) => {
            ...
            https.request("https://configsvr.msf.3g.qq.com/configsvr/serverlist.jsp?mType=getssolist", { method: "POST" }, (res) => {
                let data = [];
                res.on("error", reject);
    ...
    async resolveDns(domain, family = 4) {
    ...
    async queryServerList() {
        ...
        try {
            this.c.logger.mark("正在解析域名...");
            let addressesV4 = await this.resolveDns("msfwifi.3g.qq.com", 4);
            addressesV4 = addressesV4.slice(0, 3);
            for (let address of addressesV4) {
                host_port[address] = 8080;
            }
            let addressesV6 = await this.resolveDns("msfwifiv6.3g.qq.com", 6);
            addressesV6 = addressesV6.slice(0, 3);
            for (let address of addressesV6) {
                host_port[address] = 8080;
            }
            
        } catch (err) {
            this.c.logger.warn("解析域名失败: " + (err ? err.message : "timeout"));
        }
        if (Object.keys(host_port).length === 0) {
            throw new Error("no aliveable server");
        }
    }
}
