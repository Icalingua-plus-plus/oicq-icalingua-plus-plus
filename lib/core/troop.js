...
const { uinAutoCheck, pipeline, code2uin, NOOP } = require("../common");
...
/**
 * @this {import("./ref").Client}
 * @param {number} group_id 发送的对象
 * @returns {import("./ref").ProtocolResponse}
 */
async function groupSign(group_id) {
    [group_id] = uinAutoCheck(group_id);
    const o = {
        2: {
            1: String(this.uin),
    ...
    const body = pb.encode(o);
    const blob = await this.sendOidb("OidbSvc.0xeb7_1", body);
    const rsp = pb.decode(blob);
    return { result: rsp[3] & 0xffffffff };
}
...
async function muteAnonymous(group_id, flag, duration = 1800) {
    ...
    const cookie = this.cookies["qqweb.qq.com"];
    try {
 ...
 * @returns {import("./ref").ProtocolResponse}
 */
async function setSignature(sign = "") {
 ...
 * @returns {import("./ref").ProtocolResponse}
 */
async function getSignature(sign = "") {
    sign = Buffer.from(String(sign)).slice(0, 254);
    const body = pb.encode({
		1: this.uin,
		3: {
    ...
    const rsp = await this.sendOidbSvcTrpcTcp("OidbSvcTrpcTcp.0xfe1_2", body);
    return { result: rsp[1][2][2][2].toString() };
}
...
async function getGroupNotice(group_id) {
    [group_id] = uinAutoCheck(group_id);
    const cookie = this.cookies["qun.qq.com"];
    const url = `https://web.qun.qq.com/cgi-bin/announce/get_t_list?bkn=${this.bkn}&qid=${group_id}&ft=23&s=-1&n=20`;
    try {
 ...
 * @returns {import("./ref").ProtocolResponse}
 */
async function setGroupRemark(group_id, remark) {
    [group_id] = uinAutoCheck(group_id);
    const body = pb.encode({
        1: {
            1: group_id,
    ...
    const blob = await this.sendOidb("OidbSvc.0xf16_1", body);
    const rsp = pb.decode(blob);
    return { result: rsp[3] };
}
 ...
 * @returns {import("./ref").ProtocolResponse}
 */
async function setFriendRemark(user_id, remark) {
    [user_id] = uinAutoCheck(user_id);
    const req = jce.encodeStruct([
        user_id, remark
    ]);
    const extra = {
        req_id: this.seq_id + 1,
        service: "KQQ.ProfileService.ProfileServantObj",
        method: "ChangeFriendName",
    };
    const body = jce.encodeWrapper({ req }, extra);
    const blob = await this.sendUni("ProfileService.ChangeFriendName", body);
    const rsp = jce.decode(blob);
    return { result: rsp[0] };
}
/**
 * 设置表情回应，参考（https://bot.q.qq.com/wiki/develop/api-v2/openapi/emoji/model.html#EmojiType）
 * @this {import("./ref").Client}
 * @param {number} group_id
 * @param {number} msg_seq
 * @param {string} id
 * @param {number} type
 * @param {boolean} set
 * @returns {import("./ref").ProtocolResponse}
 */
async function setEmojiReaction(group_id, msg_seq, id, type = 1, set = true) {
    [group_id] = uinAutoCheck(group_id);
    const body = pb.encode({
        2: group_id,
        3: msg_seq,
    ...
    const cmd = set ? "OidbSvcTrpcTcp.0x9082_1" : "OidbSvcTrpcTcp.0x9082_2";
    const blob = await this.sendOidbSvcTrpcTcp(cmd, body);
    const rsp = pb.decode(blob);
    return { result: rsp[3] };
}
 ...
 * @param {string} data
 */
async function sendButtonCallback(group_id, msg_seq, appid, id, data) {
    [group_id] = uinAutoCheck(group_id);
    const body = pb.encode({
        3: appid,
        4: msg_seq,
    ...
    await this.sendOidbSvcTrpcTcp("OidbSvcTrpcTcp.0x112e_1", body);
    return { result: 0 };
}
...
module.exports = {
    ...
    setProfile, setSignature, addFriend, delFriend, setPortrait, setGroupPortrait,
    groupSign, getSignature, setGroupRemark, setFriendRemark, setEmojiReaction, sendButtonCallback
};
