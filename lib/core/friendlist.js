...
async function __getGML(group_id, next_uin) {
    const GTML = jce.encodeStruct([
        this.uin, group_id, next_uin, common.code2uin(group_id), 3, 1, common.timestamp(), 1, 1
    ]);
    const extra = {
        req_id: this.seq_id + 1,
        service: "mqq.IMService.FriendListServiceServantObj",
...
async function uid2uin(uid) {
    if (uid2uin_map.has(uid))
        return uid2uin_map.get(uid);
    for (const f of this.fl.values()) {
        if (f.uid === uid) {
            uid2uin_map.set(uid, f.user_id);
            return f.user_id;
        }
    }
    const body = pb.encode({
