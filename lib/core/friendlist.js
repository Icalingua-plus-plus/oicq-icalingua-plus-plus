...
async function _initFL(start, limit, tmp) {
    const FL = jce.encodeStruct([
        ...
        40, null, 0, 0, 0, d50, null, [13580, 13581, 13582]
    ]);
    const extra = {
    ...
    for (let v of nested[7]) {
        ...
        this.fl.set(v[0], {
            ...
            age, area,
            remark: v[3] || "",
            // group: gmap[v[1]] || "unknown",
            uid: v[63] || "",
        });
        if (v[63]) {
            this.uid2uin_map.set(v[63], v[0]);
            this.uin2uid_map.set(v[0], v[63]);
        }
        if (v[0] === this.uin &&!this.uid) this.uid = v[63];
    }
    return nested[5];
}
 ...
 * @returns {import("./ref").ProtocolResponse}
 */
async function updateFL() {
    let start = 0, limit = 150;
    while (1) {
        try {
            const total = await _initFL.call(this, start, limit, this.fl);
            start += limit;
            if (start > total) break;
        } catch (e) {
            this.logger.debug(e);
            this.logger.warn("更新好友列表出现异常。");
            return { result: -1 };
        }
    }
    return { result: 0 };
...
async function __getGML(group_id, next_uin) {
    const GTML = jce.encodeStruct([
        this.uin, group_id, next_uin, common.code2uin(group_id), 2, 0, 0, 1, 1
    ]);
    const extra = {
        req_id: this.seq_id + 1,
        service: "mqq.IMService.FriendListServiceServantObj",
    ...
    const blob = await this.sendUni("friendlist.GetTroopMemberList", body, 10);
    const nested = jce.decode(blob);
    ...
    for (let v of nested[3]) {
        ...
        if (v[42]) {
            this.uid2uin_map.set(v[42], v[0]);
            this.uin2uid_map.set(v[0], v[42]);
        }
    }
    try {
...
async function getGI(group_id, no_cache = false) {
    ...
    ginfo = {
        group_id: group_id,
        group_name: o[89] ? String(o[89]) : String(o[15]),
        group_remark: o[110] ? String(o[110]) : "",
        member_count: o[6],
...
async function getGMI(group_id, user_id, no_cache = false) {
    ...
    const card = o[8] ? common.parseFunString(o[8].toBuffer()) : "";
    minfo = {
        group_id: group_id,
        user_id: user_id,
        nickname: Reflect.has(o, "11") ? String(o[11]) : "",
        card: card,
        sex: Reflect.has(o, "9") ? group_sex_map[o[9] & 0xffffffff] : "unknown",
        age: o[12] || 0,
        area: Reflect.has(o, "10") ? String(o[10]) : "unknown",
        join_time: o[14],
        last_sent_time: o[15],
        level: o[39],
        rank: Reflect.has(o, "13") ? String(o[13]) : undefined,
        role: group_role_map[o[27]],
        unfriendly: false,
        title: Reflect.has(o, "31") ? String(o[31]) : "",
    ...
    try {
        if (!minfo.uid) this.getGroupMemberList(group_id, true);
        try {
 ...
 */
async function uid2uin(uid) {
    if (Number.isInteger(Number(uid))) return Number(uid); // 纯数字的uid是误调用的uin，直接返回
    if (this.uid2uin_map.has(uid))
        return this.uid2uin_map.get(uid);
    const body = pb.encode({
        1: uid,
        2: 0,
        3: [{ 1: 20002 }],
    });
    const blob = await this.sendOidbSvcTrpcTcp("OidbSvcTrpcTcp.0xfe1_2", body, true);
    const user_id = blob[1][3];
    this.uid2uin_map.set(uid, user_id);
    this.uin2uid_map.set(user_id, uid);
    return user_id;
}

module.exports = {
    initFL, initGL,
    getGML, getGI, getGMI, getSI,
    updateFL,
    uid2uin,
};
