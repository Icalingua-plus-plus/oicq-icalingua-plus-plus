...
const { genC2CMessageId, genGroupMessageId, log, uin2code, BUF0, timestamp } = require("./common");
...
const sub0x27 = {
    ...
    60: async function (data, time) {
        let user_id = data[10][1];
        if (this.config.useNT) {
            user_id = await uid2uin.call(this, String(user_id));
        }
    ...
    21: async function (data, time) {
        if (data[11] && data[11][1] === 1) {
            const user_id = data[11][4]; //这里NT上线也是uin，不需要转换
            this.em("notice.group.setting", {
...
const push528 = {
    0x8A: async function (buf, time) {
        ...
        if (user_id === this.uin || String(user_id) === this.uid) {
            user_id = data[2];
            flag = 1;
        }
        if (this.config.useNT) {
 ...
 */
function onGroupSetting(group_id, field, enable, time, user_id) {
    if (!field) return;
    const e = {
        group_id, time, user_id
    };
    e[field] = !!enable;
 ...
 */
const push732 = {
    0x0C: async function (group_id, buf, time) {
        let operator_id, user_id, duration;
        if (this.config.useNT) {
            const o = pb.decode(buf);
            operator_id = await uid2uin.call(this, String(o[4]));
            user_id = o[5][3][1] ? await uid2uin.call(this, String(o[5][3][1])) : 0;
            duration = o[5][3][2];
        } else {
            operator_id = buf.readUInt32BE(6);
            user_id = buf.readUInt32BE(16);
    ...
    0x11: async function (group_id, buf, time) {
        ...
        if (this.config.useNT) {
            //nt uid
            user_id = await uid2uin.call(this, String(user_id));
            operator_id = await uid2uin.call(this, String(operator_id));
        }
    ...
    0x15: function (group_id, buf, time) {
        ...
        try {
            ...
            const random = data[3];
            const add = data[4] === 1;
            const user_id = data[5]; //这里NT上线也是uin，不需要转换
            const operator_id = data[6]; //这里NT上线也是uin，不需要转换
    ...
    0x10: async function (group_id, buf, time) {
        if (buf[6] === 0x26) {
        ...
        switch (sub_type) {
            case 23: { // 群荣誉相关设置
                ...
                for (let field of Object.keys(field_map)) {
                    if (data[field] != null) {
                        const enable = data[field] === 0;
                        let user_id = sub[21];
                        if (this.config.useNT) {
                            user_id = await uid2uin.call(this, String(user_id));
                        }
                        onGroupSetting.call(this, group_id, field_map[field], enable, time, user_id);
                    }
                }
                return;
            }
            case 12: { // 群名称修改
                ...
                const group_name = String(sub[5][2]);
                try {
                    this.gl.get(group_id).group_name = group_name;
                } catch (e) { }
            ...
            case 35: { // 消息回应
                const data = sub[44];
                let user_id = data[1][1][3][4];
                if (this.config.useNT) {
                    user_id = await uid2uin.call(this, String(user_id));
                }
                const set = data[1][1][3][5] === 1;
                return this.em("notice.group.reaction", {
                    group_id, time,
                    user_id: user_id,
            ...
            case 38: { // 机器人变动
                const data = sub[47];
                const is_join = !!data[2];
                let user_id = is_join ? data[2] : data[3];
                let nickname = "";
                user_id = await uid2uin.call(this, String(user_id)); // 这里全是uid，老上线也是
                try {
                    if (is_join) {
                        ...
                        const info = await this.getGroupMemberInfo(group_id, user_id);
                        nickname = info.data.nickname || info.data.card || "";
                        try {
                        ...
                        this.em("notice.group.decrease", {
                            group_id, user_id,
                            operator_id: user_id, dismiss: false,
                            member, group, time
                        });
                    }
                } catch (e) {
        ...
        if (sub[5] && sub[5][2]) {
            ...
            if (str.includes("获得群主授予的")) {
                const user_id = sub[5][5]; // 这里NT上线也是uin，不需要转换
...
function onlinePushListener(blob, seq) {
    ...
    for (let v of list) {
        ...
        if (this.config.useNT) continue; // NT上线忽略老推送
        const time = v[5];
        if (v[2] === 528) {
...
function onlinePushTransListener(blob, seq) {
    ...
    if (this.config.useNT) return; // NT上线忽略老推送
    const time = push[8];
...
async function groupMsgListener(blob, seq) {
    ...
    try {
        ...
        if (content[1] > 1) {
            ...
            msg = rebuildFragments(msg, group_id, this.uin, user_id);
            if (!msg)
                return;
        } else {
            //非分片消息
 ...
 */
function rebuildFragments(msg, chat_id, self_id, user_id) {
    const head = msg[1], content = msg[2], body = msg[3];
    const cnt = content[1] || 0, index = content[2] || 0, div = content[3] || 0;
    const id = self_id + "-" + user_id + "-" + chat_id + "-" + div; //self_id-user_id-chat_id-div
    if (!FRAG.has(id)) {
 ...
 * @this {import("./ref").Client}
 */
async function NTMsgListener(blob, seq, sso) {
    const o = pb.decode(blob);
    ...
    if (o[4]) {
        let trans_info = BUF0;
        try {
            const trans_info = pb.decode(sso.reserved)[23] || BUF0;
        } catch (e) {
            this.logger.debug(e);
        }
    ...
    switch (type) {
        ...
        case 33: //群员入群
            try {
                const group_id = uin2code(o[1][1][1]);
                const msg = o[1][3][2];
                const user_uid = String(msg[3]);
                const user_id = await uid2uin.call(this, user_uid);
                let nickname = "";
                const ginfo = (await this.getGroupInfo(group_id)).data;
                if (!ginfo) return;
                if (user_id === this.uin) {
        ...
        case 529: //离线文件
            ...
            try {
                ...
                const user_id = msg[1][1];

                if (msg[2][1] > 1) {
                    //重组分片消息
                    msg = rebuildFragments(msg, -user_id, this.uin, user_id);
                    if (!msg)
                        return;
                } else {
                    //非分片消息
        ...
        case 44: //群管理权限变动
            try {
                const group_id = uin2code(o[1][1][1]);
                const msg = o[1][3][2], time = o[1][2][6];
                const type = msg[3];
                if (type === 0 || type === 1) {
                    const data = msg[4][type === 0 ? 1 : 2];
                    const user_id = await uid2uin.call(this, String(data[1]));
                    const set = data[2] === 1;
                    try {
                        (await this.getGroupMemberInfo(group_id, user_id)).data.role = (set ? "admin" : "member");
                    } catch (e) { }
                    ...
                    const operator_id = await uid2uin.call(this, String(msg[4][3][1])),
                        user_id = await uid2uin.call(this, String(msg[4][3][2]));
                    try {
                        this.gl.get(group_id).owner_id = user_id;
                        (await this.getGroupMemberInfo(group_id, operator_id)).data.role = "member";
                        (await this.getGroupMemberInfo(group_id, user_id)).data.role = "owner";
        ...
        case 34: //群成员离开
            try {
                const group_id = uin2code(o[1][1][1]);
                const msg = o[1][3][2], time = o[1][2][6];
                const user_id = await uid2uin.call(this, String(msg[3]));
                const exit_type = msg[4];
                let operator_id, dismiss = false, member, group;
                try {
                    member = this.gml.get(group_id).get(user_id);
                } catch { }
                if (exit_type === 0x82 || exit_type === 0x2) {
                    operator_id = user_id;
                    try {
                        this.gml.get(group_id).delete(user_id);
                    } catch { }
                } else {
                    operator_id = msg[5] && msg[5][1] ? await uid2uin.call(this, String(msg[5][1][1])) : user_id;
                    if (exit_type === 0x01 || exit_type === 0x81)
                        dismiss = true;
                    if (user_id === this.uin) {
                        group = this.gl.get(group_id);
                        this.gl.delete(group_id);
