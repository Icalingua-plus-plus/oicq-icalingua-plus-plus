...
const { updateFL } = require("./core/friendlist");
...
const sub0x27 = {
    ...
    21: function (data, time) {
        if (data[11][1] === 0) {
            ...
            onGroupSetting.call(this, data[11][3], "avatar", true, time);
        }
    }
};
...
const push528 = {
    0x8A: function (buf, time) {
        ...
        if (this.config.useNT) {
            ...
            let fl = this.fl.values();
            for (let member of fl) {
            ...
            fl = this.fl.values();
            for (let member of fl) {
                if (member.uid === operator_id) {
                    operator_id = member.user_id;
                    break;
                }
            }
        }
...
const push732 = {
    ...
    0x11: function (group_id, buf, time) {
        ...
        if (this.config.useNT) {
            //nt uid
            user_id = String(user_id), operator_id = String(operator_id);
            let group_list = this.gml.get(group_id).values();
            for (let member of group_list) {
                if (member.uid === user_id) {
                    user_id = member.user_id;
                    break;
                }
            }
            group_list = this.gml.get(group_id).values();
            for (let member of group_list) {
                if (member.uid === operator_id) {
                    operator_id = member.user_id;
                    break;
...
async function NTMsgListener(blob, seq) {
    ...
    switch (type) {
        case 82: //群聊消息
            try {
                const msg = groupNTMsgConverter.call(this, o);
                //生成消息id
                const head = msg[1], content = msg[2], body = msg[3];
                if (head[9][2] === 127) return;
                const user_id = head[1], time = head[6], seq = head[5];
        ...
        case 529: //离线文件
            ++this.stat.recv_msg_cnt;
            try {
                const msg = c2cNTMsgConverter.call(this, o);
        ...
        case 528: //push528
            try {
                const type = o[1][2][3], buf = o[1][3][2]._raw, time = o[1][2][6];
...
function groupNTMsgConverter(msg) {
    const headerNT = msg[1][1], contentNT = msg[1][2], bodyNT = msg[1][3];
    const header = pb.decode(headerNT._raw), content = pb.decode(contentNT._raw), body = pb.decode(bodyNT._raw);
...
module.exports = {
    onlinePushListener, onlinePushTransListener, c2cMsgSyncListener, groupMsgListener, discussMsgListener, NTMsgListener
};
